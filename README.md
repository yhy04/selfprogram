...
复杂度分析
```
时间复杂度：O(mnlogm)     空间复杂度：O(m)
```
#### 3082 hard  
复杂度分析
```
时间复杂度：O(kn^2)          空间复杂度：O(nk)
```
#### 3485 hard  
复杂度分析
```
...
## base_knowledge - 对计算机原理的理解

### 项目介绍
采用现代C++编写,使用代码展示计算机基本原理,包括内存
...
#### hello_world.cpp
数据类型的本质是程序对内存段的不同解读  
使用int数组在内存段上的比特值,打印"hello,world"  
小段存储与大端存储会产生不同结果
